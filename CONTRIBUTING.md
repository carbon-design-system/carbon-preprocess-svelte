...
3. Run three extractors. Each one gates what it adds so the index stays tight:
   ...
   - [`extract-runtime-classes.ts`](src/indexer/extract-runtime-classes.ts) follows the module import graph from each component: `classList.add/remove/toggle("bx--…")` calls in `.svelte` modules, and every `bx--` class a `.js` module applies (hoisted constants, class prefixes). Lookup selectors in `.js` (`closest(".bx--modal")`) are skipped, since a shared utility would otherwise hand the looked-up component's classes to every importer.
