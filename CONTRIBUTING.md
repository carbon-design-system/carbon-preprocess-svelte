...
2. List every `.svelte`/`.js` under `src/` (via an injectable `listFiles`, see below), parsing markup with `svelte/compiler` + `estree-walker` to pull static classes, sub-components, slot wrappers, and imports.
...
[`scripts/index-components.ts`](scripts/index-components.ts) is now a thin CLI wrapper: it calls `buildComponentIndex()` with a Bun `Glob`-based `listFiles`, applies `MANUAL_OVERRIDES`, and writes `src/component-index.ts`. It injects Bun's `Glob` specifically to keep regeneration byte-for-byte reproducible — see the `listFiles` doc comment on `buildComponentIndex` for why.

Set `DEBUG_INDEX=1` to print per-stage timings.

### Live index (experimental)

`optimizeImports`, `optimizeCss`, and `OptimizeCssPlugin` all accept `experimental: { liveIndex: true }`. When set, [`src/indexer/live-index.ts`](src/indexer/live-index.ts) calls `buildComponentIndex()` against *the consuming project's* installed `carbon-components-svelte` (resolved via [`resolve-carbon-root.ts`](src/indexer/resolve-carbon-root.ts), using `require.resolve.paths()` rather than a package.json subpath — Carbon's `exports` map doesn't declare `./package.json`, so a direct `require.resolve` throws under strict Node ESM even though it works under Bun) instead of using the bundled static index. The result is cached at `node_modules/.cache/carbon-preprocess-svelte/<carbon-version>.json` and falls back to the static index on any failure.

[`src/component-index-registry.ts`](src/component-index-registry.ts) (`getComponents`/`setComponents`) holds whichever index is active; every consumer reads through it instead of importing `src/component-index.ts` directly. This is **process-wide global state by design** — every plugin instance in one build shares one active index, which is correct for the common case (one Carbon version per build) but means mixing a `liveIndex: true` instance with a default instance in the same process affects both. `src/plugins/strict-css-optimizer.ts`'s `SHARED_CLASSES` cache is lazily computed on first use (not at module load) for the same reason: it must reflect a `liveIndex` swap, and that swap is guaranteed to happen (in `buildStart`/before `processAssets`) before strict-mode optimization ever runs.
