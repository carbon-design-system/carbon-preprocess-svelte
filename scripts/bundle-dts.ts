...
export async function bundleDts(opts: {
  root: string;
  source: string;
  ambientRoots?: string[];
  outFile: string;
}): Promise<void> {
  const { root, source, ambientRoots = [], outFile } = opts;
  const srcRoot = join(root, "src");
  const emitted = emitDeclarations([source, ...ambientRoots], srcRoot);
  const entryDts = sourceToDtsPath(source, srcRoot, emitted);
  const text = rollupDts(entryDts, emitted);
  await mkdir(dirname(outFile), { recursive: true });
  await writeFile(outFile, text.endsWith("\n") ? text : `${text}\n`);
}
...
function sourceToDtsPath(
  source: string,
  srcRoot: string,
  emitted: Map<string, string>,
): string {
  const rel = relative(srcRoot, source)
    .replace(TS_EXT_RE, ".d.ts")
    .replaceAll("\\", "/");
  const exact = norm(join(srcRoot, rel));
  if (emitted.has(exact)) return exact;
  for (const key of emitted.keys()) {
    if (key.endsWith(`/${rel}`)) return key;
  }
  throw new Error(
    `no declaration emit for ${source} (expected …/${rel})\nhave: ${[...emitted.keys()].join("\n")}`,
  );
}
...
function rollupDts(entryDts: string, emitted: Map<string, string>): string {
  ...
  for (const file of reachable) {
    ...
    for (const stmt of sf.statements) {
      ...
      const decl: Decl = {
        names,
        text,
        leadingComment: getLeadingComment(stmt, sf),
        node: stmt,
        sourceFile: sf,
      };
      allDecls.push(decl);
  ...
  for (const decl of allDecls) {
    ...
    for (const alias of aliases) {
      chunks.push(
        decl.leadingComment +
          rewriteInlineImportTypes(formatPublicDecl(decl, alias)),
      );
    }
  }
      ...
      if (stmt.exportClause && ts.isNamedExports(stmt.exportClause)) {
        for (const el of stmt.exportClause.elements) {
          ...
          if (
            ...
            const remote = resolveNameInFile(
              resolved,
              localName,
              emitted,
              declsByFileAndName,
        ...
        const spec = stmt.moduleSpecifier.text;
        if (isRelative(spec)) {
          collectPublicBindings(
            resolveDts(entryDts, spec, emitted),
            emitted,
            declsByFileAndName,
            markPublic,
            stack,
...
/** Leading JSDoc/comments immediately preceding a statement, e.g. doc comments on an export. */
function getLeadingComment(stmt: ts.Statement, sf: ts.SourceFile): string {
...
function resolveDts(
  ...
  const candidates = [
    norm(join(dir, `${bare}.d.ts`)),
    norm(join(dir, bare, "index.d.ts")),
  ];
  ...
  for (const c of candidates) {
    ...
    for (const key of emitted.keys()) {
      if (key === tail || key.endsWith(`/${tail}`)) return key;
    }
  }
  throw new Error(
    `cannot resolve "${spec}" from ${fromFile}\ntried: ${candidates.join(", ")}`,
  );
}
